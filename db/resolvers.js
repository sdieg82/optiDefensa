...
const resolvers = {
    ...
    Mutation: {
        ...
        actualizarProductoCompra: async (_, {id, input}) => {
            ...
            console.log(producto.cantidadCompra);  

            const productoE=await producto.save();
            return productoE;
        }, 
        ...
        nuevoCliente: async (_, { input }, ctx) => {
            ...
            if(cliente) {
                throw new Error('El cliente con el mail ' + email + ' ya se encuentra registrado');
            }
            const clienteCedula = await Cliente.findOne({ cedula });
            if(clienteCedula) {
                throw new Error('El cliente con la cédula ' + cedula + ' ya se encuentra registrado');
            }
     ...
     if(cedula.length == 10){
        ...
        if( digito_region >= 1 && digito_region <=24 ){
          ...
          if(digito_validador == ultimo_digito){
            console.log('La cédula: ' + cedula + ' es correcta');
          }else{
            throw new Error('El número de cédula: ' + cedula + ' es incorrecta');
          }
          
        }else{
          // imprimimos en consola si la region no pertenece
          throw new Error('La cédula ingresada no pertenece a ninguna region');
        }
     }else{
        //imprimimos en consola si la cedula tiene mas o menos de 10 digitos
        throw new Error('La cédula ingresada tiene menos de 10 Digitos');
    }    
     try {
